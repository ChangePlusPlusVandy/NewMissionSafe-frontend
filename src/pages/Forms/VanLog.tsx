...
import { Button, Select, TextInput, FileInput, Group, Flex, Paper, Title, Space, RadioGroup, Radio } from '@mantine/core';
...
const schema = Yup.object().shape({
    ...
    startingMilage: Yup.mixed().required("Starting Mileage is Required").test('fileFormat', allowedFileMessage, isImageFile),
    endingMilage: Yup.mixed().required("Ending Mileage is Required").test('fileFormat', allowedFileMessage, isImageFile),
    startingGasTank: Yup.mixed().required("Starting Gas Tank is Required").test('fileFormat', allowedFileMessage, isImageFile),
    endingGasTank: Yup.mixed().required("Ending Gas Tank is Required").test('fileFormat', allowedFileMessage, isImageFile),
    wasCleanInitially: Yup.string().required("Required Question"),
    initialVanImage: Yup.mixed().test('fileFormat', allowedFileMessage, isImageFile),
    notCleanExplanation: Yup.string(),
    wasCleanOnReturn: Yup.string().required("Required Question"),
    cleanVanImage: Yup.mixed().required("Clean Van Image After Trip is Required").test('fileFormat', allowedFileMessage, isImageFile),
    additionalImage1: Yup.mixed().test('fileFormat', allowedFileMessage, isImageFile),
    additionalImage2: Yup.mixed().test('fileFormat', allowedFileMessage, isImageFile),
    additionalImage3: Yup.mixed().test('fileFormat', allowedFileMessage, isImageFile)
});
                ...
                <Paper w={"70%"} bg={"missionSafeBlue.9"}>
                    <Flex direction="column"
                        ...
                        <form onSubmit={form.onSubmit(submit, console.log)}>
                            ...
                            >
                                <Group>
                                <Radio label="Yes" value="yes"  styles={{ label: { color: 'white' } }}/>
                                <Radio label="No" value="no"  styles={{ label: { color: 'white' } }}/>
                                </Group>
                            </RadioGroup>
                            <FileInput
